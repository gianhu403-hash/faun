...
- `faun/embeddings` — **единый владелец** батч-экспорта эмбеддингов: протокол `Embedder` + `PerchEmbedder` (1280, 32k/окно 160000) и `YamnetEmbedder` (2048, 16k/concat(mean,max)) поверх `experiments/wrappers/{perch,yamnet_probe}` (ленивый TF), `embed_batch`, `EmbeddingCache` (npz). Дублировать embedding-export запрещено.
...
- `faun/cli` — `faun process <dir> [--out results.csv]`; `faun retrain --labels <csv> --model yamnet --out probe.pkl`; `faun export-clips --job <dir> --out clips.zip`; `faun batch-label --archive <dir> --out <jsonl> [--emb-out npz] [--embedder perch|yamnet] [--models perch,birdnet]`; `faun fetch-dataset --root <iNatSounds>`; `faun eval-species --probe <pkl> --dataset <dir> [--embedder perch|yamnet] [--real]` (без `--real` число помечается SYNTHETIC).
