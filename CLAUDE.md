...
- `faun/settings` — `Settings`(frozen) + `get_settings()` (кэш, `cache_clear()` в тестах): единый типизированный конфиг, **читается через `get_settings()`** в `api`/`sources`/адаптерах(`perch`/`perch_v2`/`yamnet`)/`health`; +`basic_user`/`basic_pass` (Basic Auth, verbatim-чтение через `_env_secret_opt`). source-лимиты enforce-ит `faun.sources` (uncompressed-cap — через `_int_env` с дефолтом из settings, чтобы env-override и zip-bomb-тест работали). `faun/obs` — `setup_logging(json)` + `with_job_context(job_id)` (структурные JSON-логи, stdlib).
