"""Tests for faun.localization.triangulate — TDOA estimation and triangulation.
...
from faun.localization.distance import (
    DistanceEstimate,
    estimate_distances,
    _rms_energy,
    _DEFAULT_REF_RMS,
...
# ---------------------------------------------------------------------------
# localize_event — honest event-localization entry point
# ---------------------------------------------------------------------------


class TestLocalizeEvent:
    """Tests for the trap-keyed localize_event entry point."""

    def test_three_traps_localizes(self, triangle_mics: list[MicPosition]) -> None:
        """3 traps with a synthetic source at the centroid -> localized result."""
        lat0, lon0 = triangle_mics[0].lat, triangle_mics[0].lon
        mics_xy = [(0.0, 0.0)]
        for m in triangle_mics[1:]:
            mics_xy.append(_latlon_to_meters(lat0, lon0, m.lat, m.lon))
        ...
        signals = _simulate_source(centroid, mics_xy, snr_db=40, seed=7)
        trap_ids = ["A1", "A2", "A3"]
        signals_by_trap = dict(zip(trap_ids, signals))
        trap_positions = dict(zip(trap_ids, triangle_mics))

        result = localize_event(signals_by_trap, trap_positions, sr=16000)
        assert result.method == "tdoa-synthetic-validated"
        assert result.n_traps == 3
        assert result.lat is not None
        assert result.error_m < 50.0, f"error_m {result.error_m:.1f} (want <50)"

    def test_two_traps_insufficient(self, triangle_mics: list[MicPosition]) -> None:
        """2 traps -> honest sentinel, never a fabricated centroid."""
        trap_ids = ["A1", "A2"]
        signals_by_trap = {tid: np.zeros(16000, dtype=np.float32) for tid in trap_ids}
        trap_positions = dict(zip(trap_ids, triangle_mics[:2]))

        result = localize_event(signals_by_trap, trap_positions, sr=16000)
        assert result.method == "insufficient-traps"
        assert result.lat is None
        assert result.lon is None
        assert result.n_traps == 2
