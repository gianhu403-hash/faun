"""TDOA triangulation of sound sources (ported from the v1 hackathon edge core).

Method: hyperbolic multilateration. Each mic pair's TDOA constrains the
*difference* of distances to the two mics; equivalently it is a family of
growing circles ``c * (tau - t0)`` whose intersection is the source. We fuse
those hyperbolic constraints with soft energy-based (inverse-square) distance
constraints and solve by multi-start Nelder-Mead.

Accuracy is tied to the sample rate: at 16 kHz one sample is 62.5 microseconds,
about 21 mm of sound travel, and a 100 m hop between traps takes about 291 ms —
but real trap timestamps are second-grained, roughly 16000x coarser than the
per-sample resolution and far too coarse to resolve TDOA. So this core is
validated on SYNTHETIC signals only; on real trap data localization is blocked
pending millisecond clock sync. That honesty is why the method tag is
``"tdoa-synthetic-validated"``.
"""
...
def triangulate(
    ...
    Method: hyperbolic multilateration. Each mic pair's TDOA constrains the
    *difference* of distances to the two mics — equivalently a family of
    growing circles ``c * (tau - t0)`` whose intersection is the source. This
    is fused with energy-based ranging that constrains the *absolute* distance
    to each mic. Neither is accurate alone, but together they narrow the
    uncertainty region significantly.

    Accuracy is tied to the sample rate: at 16 kHz one sample is 62.5 microseconds
    (about 21 mm of sound travel), and a 100 m hop between traps takes about 291 ms
    — yet real trap timestamps are second-grained, roughly 16000x coarser than the
    per-sample resolution. Hence this is validated on
    SYNTHETIC signals only; on real data it is blocked pending millisecond clock
    sync. That honesty is encoded in the ``"tdoa-synthetic-validated"`` method
    tag set on the result.
    ...
    result = TriangulationResult(lat=src_lat, lon=src_lon, error_m=error_m)
    result.method = "tdoa-synthetic-validated"
    result.n_traps = n
    return result


def localize_event(
    signals_by_trap: dict[str, np.ndarray],
    trap_positions: dict[str, MicPosition],
    sr: int,
    speed_of_sound: float = SPEED_OF_SOUND,
) -> TriangulationResult:
    """Localize one acoustic event from per-trap signals via TDOA multilateration.

    Method: hyperbolic multilateration (see :func:`triangulate`). Each trap pair's
    TDOA constrains the difference of distances — equivalently growing circles
    ``c * (tau - t0)`` whose intersection is the source.

    Accuracy is tied to the sample rate: at 16 kHz one sample is 62.5 microseconds
    (about 21 mm of sound travel), and a 100 m hop between traps takes about 291 ms,
    while real trap timestamps are second-grained (roughly 16000x coarser than the
    per-sample resolution). So this is validated on SYNTHETIC
    signals only; on real trap data it is blocked pending millisecond clock sync —
    which is exactly why a successful result is tagged ``"tdoa-synthetic-validated"``.

    Parameters
    ----------
    signals_by_trap : dict[str, np.ndarray]
        Map trap id -> 1-D event signal at that trap.
    trap_positions : dict[str, MicPosition]
        Map trap id -> mic position.
    sr : int
        Sample rate of the signals (Hz).
    speed_of_sound : float
        Speed of sound (m/s). If it differs from the 343.0 default, an equivalent
        temperature is derived and passed to :func:`triangulate` so the speed is
        honored.

    Returns
    -------
    TriangulationResult
        With fewer than 3 traps in common, returns an honest sentinel
        (``lat=None, lon=None, error_m=nan, method="insufficient-traps"``) rather
        than fabricating a centroid. Otherwise a localized result with
        ``method="tdoa-synthetic-validated"``.
    """
    trap_ids = sorted(set(signals_by_trap) & set(trap_positions))
    k = len(trap_ids)
    ...
    if k < 3:
        return TriangulationResult(
            ...
            lon=None,
            error_m=float("nan"),
            method="insufficient-traps",
            n_traps=k,
        )

    signals = [signals_by_trap[tid] for tid in trap_ids]
    mics = [trap_positions[tid] for tid in trap_ids]

    if speed_of_sound != SPEED_OF_SOUND:
        temperature_c: float | None = (speed_of_sound - 331.3) / 0.606
    else:
        temperature_c = None

    result = triangulate(signals, mics, sample_rate=sr, temperature_c=temperature_c)
    result.n_traps = k
    return result
