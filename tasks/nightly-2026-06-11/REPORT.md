...
| **ESC-50 probe** | Perch **0.9962**, YAMNet **0.9969** AUC | chirping_birds vs rest, 5-fold CV, frozen embeddings. Контрастная задача — обе почти идеальны |
| **E2 Perch (ff1010)** ☀️добито утром | roc_auc **0.905** | N=400, 5-fold CV — лучший на различающей bird/no-bird задаче |
| **E5 YAMNet-base (ff1010)** ☀️ | roc_auc **0.860** | та же постановка — уступает Perch 4.5 пункта |
| **E1 BirdNET-детекция (ff1010)** ☀️ | roc_auc **0.672** | BirdNET слаб как ДЕТЕКТОР присутствия (он — видовой классификатор) → подтверждает двухэтапную архитектуру |
| E10 few-shot | skip (ожидаемо) | нет ключа xeno-canto |

**Рекомендация модели (теперь с эмпирией):** Perch 2 (Apache 2.0) как продуктовый классификатор — Perch v1 уже обошёл YAMNet-base на различающей задаче (0.905 vs 0.860), v2 ожидаемо сильнее; BirdNET — видовая инвентаризация на выделенных событиях, но **лицензия CC BY-NC-SA** (не «BY-NC», как считали: ShareAlike заражает дообученные головы) — для коммерческого пилота нужна альтернатива или договорённость, и как детектор он слаб (0.672). Детектор: onset.py stage-1 + CLAP-верификатор опционально. Полная матрица: `experiments/report/research-report.md`.
...
## Автономный хвост (после «доведи до конца», ~10:35–11:00)
- **E1/E2/E5 добиты на ff1010** (7690 WAV распакованы; образ faun-ml-cpu пересобран с tensorflow_hub/tf-keras/setuptools<81, гейт hub-ok) — реальные AUC в таблице выше.
- **Job-store унифицирован**: api.py теперь живёт на faun.jobs (атомарный стор), дубль-хелперы удалены, внешний JSON-контракт без изменений, +1 тест (сюита 181).
- **UI**: поллинг больше не крутится молча при ошибках (3 фейла → предупреждение, 10 → стоп); CSV-парсер таблицы понимает кавычки.
- **exp_esc50_probe.py** перенесён с кластера в `experiments/`.
- **bench CSV** (`docs/results/bioacoustics_bench.csv`) — только реальные метрики; полный честный лог со skip-строками — `experiments/report/results.csv`.
- **Ветка docs/mkdocs-technical-documentation**: заархивирована тегом `archive/docs-mkdocs-technical-documentation` (контент сохранён), ветка удалена — фикс был для замороженного legacy.
- Хвост на кластер: `resampy` нужен birdnetlib — пока в /data/pylibs, добавить в Dockerfile при следующей пересборке; warblrb10k докачивался (второй корпус для кросс-проверки — днём).
