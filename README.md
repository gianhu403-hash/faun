...
edge — запускает edge.server. Слушает микрофоны (реальные или симулятор), при обнаружении резкого звука (onset detection) → классифицирует звук через YAMNet → триангулирует через GCC-PHAT → принимает решение (ALERT/CHECK/IGNORE) → отправляет дрон → передаёт данные на gateway через LoRa.
